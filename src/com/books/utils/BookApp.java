...
public abstract class BookApp extends App {
    ...
    protected void config() {
        ...
        D.p(String.format("%s 上次扫描参数为:%s", SITE, lastScanCateInfo == null ? "null" : lastScanCateInfo.toString()));
    }
    ...
    protected int pageCountOff(int currentCount, String cate) {
        ...
        D.p(String.format("%s 上次页数:%d  此次页数:%d  预计扫描%d", SITE, lastPage, currentCount, scanCount));
    ...
    public boolean saveScanInfo() {
        ...
        D.p(SITE + "  保存扫描数据==>" + scanInfoModel.toString());
